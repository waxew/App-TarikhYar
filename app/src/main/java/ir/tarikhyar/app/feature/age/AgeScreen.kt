        ...
        LazyColumn(
            ...
            item {
                ResultCard {
                    ...
                    PrimaryButton("انتخاب تاریخ بدون تایپ") { showPicker = true }
                    ErrorText(error)
            ...
            item {
                AnimatedVisibility(result != null && birth != null) {
                    if (result != null && birth != null) AgeResultCard(birth!!, today, result!!, context)
                }
            }
        }
    }
    if (showPicker) {
        val initial = parsePersianDate(year, month, day) ?: PersianCalendar.addYears(today, -25)
        PersianDatePickerDialog(initial, { showPicker = false }) {
...
@Composable
private fun AgeResultCard(birth: PersianDate, today: PersianDate, result: DateCalculations.AgeResult, context: android.content.Context) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ResultCard {
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(14.dp)) {
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text("${PersianFormat.digits(result.period.years)} سال", style = MaterialTheme.typography.headlineLarge, color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.ExtraBold)
                    Text("${PersianFormat.digits(result.period.months)} ماه و ${PersianFormat.digits(result.period.days)} روز", color = MaterialTheme.colorScheme.onSurface)
                }
            }
            ...
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(14.dp)) {
                Box(contentAlignment = Alignment.Center, modifier = Modifier.size(76.dp)) {
                    CircularProgressIndicator(progress = { progress }, modifier = Modifier.fillMaxSize(), color = MaterialTheme.colorScheme.primary, trackColor = Color(0xFFFFD8DE), strokeWidth = 7.dp)
                    Text("${(progress * 100).toInt()}%", fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.primary)
                }
                Column(modifier = Modifier.weight(1f)) {
                    Text("تا تولد بعدی شما", color = MaterialTheme.colorScheme.onSurface, fontWeight = FontWeight.Bold)
                    Text(if (result.nextBirthdayDays == 0L) "امروز 🎉" else "${PersianFormat.number(result.nextBirthdayDays)} روز", style = MaterialTheme.typography.titleLarge, color = MaterialTheme.colorScheme.primary)
                }
        ...
        ResultCard {
            Text("کارت تصویری", style = MaterialTheme.typography.titleLarge, color = MaterialTheme.colorScheme.primary)
            PrimaryButton("ساخت و اشتراک کارت نتیجه") {
                ShareCardRenderer.share(context, "نتیجه سن من", listOf("${PersianFormat.digits(result.period.years)} سال، ${PersianFormat.digits(result.period.months)} ماه و ${PersianFormat.digits(result.period.days)} روز", "تولد: ${PersianFormat.persianDate(birth)}", "${PersianFormat.number(result.totalDays)} روز زندگی"))
            }
        }
        ...
        ResultCard {
            ...
            Text(if (daysToLegal <= 0) "۱۸ سالگی کامل شده است." else "${PersianFormat.number(daysToLegal.toLong())} روز تا ۱۸ سالگی", color = MaterialTheme.colorScheme.onSurfaceVariant)
        }
    }
}
...
private fun DetailLine(label: String, value: String, emoji: String) {
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
        ...
        Text(value, fontWeight = FontWeight.Bold, color = MaterialTheme.colorScheme.onSurface, textAlign = TextAlign.End)
    }
}
